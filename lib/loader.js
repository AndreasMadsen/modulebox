...
(function (undefined) {
  ...
  function ModuleBox(settings) {
    ...
    // Save url encoder
    settings = settings || {};
    this._baseUrl = settings.baseUrl || window.location.protocol + '//' + window.location.host + '/modulebox/';
  ...
  ModuleBox.prototype._generateSourcePath = function (filepath, special) {
    return this._sourcePath + (special ? '_special_/' : '') + filepath.slice(1);
  };
  ...
  ModuleBox.prototype._request = function (from, requests, callback) {
    ...
    xhr.onreadystatechange = function () {
      ...
      for (var j = 0; j < requests.length; j++) {
        ...
        var value = unpackValue(special ? specialResolve[input] : normalResolve[input]);
      ...
      for (var i = 0, l = files.length; i < l; i++) {
        ...
        for (var k = 0; k < keys.length; k++) {
          mapTransform[keys[k]] = {
            'special': self._specialMap[keys[k]] || special,
            'value': unpackValue(mapOrigin[keys[k]])
          };
        }
  ...
  function unpackValue(value) {
    return (typeof value === 'string') ? value : createError(value);
  }
