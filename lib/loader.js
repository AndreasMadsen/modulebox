...
(function (undefined) {
  ...
  function ensure(name, callback) {
    ...
    // Already ensured
    var resolved = this._resolved[name];
    if (resolved) {
    ...
    if (!resolved) {
      if (console && console.error) {
        console.error(new Error(
          'Warning: ' + name + ' was requested synchronously from ' +
                        this._filename
        ));
      }

      resolved = list[name] = this._box._request(this._filename, name);
    }
