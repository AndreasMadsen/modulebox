...
(function (undefined) {
  ...
  function ModuleBox(settings) {
    ...
    this._sourceUrl = settings.source || function (filepath, special) {
      return '/modulebox' + (special ? '/__special__' : '') + filepath;
    };
  ...
  ModuleBox.prototype._request = function (from, requests, callback) {
    ...
    var destination = this._urlEncode(
      this._normal.fetched, this._special.fetched, from, requests
    );
    var syncReturn;
    var async = !!callback;
    ...
    xhr.onreadystatechange = function () {
      ...
      for (var j = 0; j < requests.length; j++) {
        ...
        var value = special ? specialResolve[input] : normalResolve[input];
            value = (typeof value === 'string') ? value : createError(value);

        transform[input] = { 'special': special, 'value': value };
        if (special) self._specialMap[input] = value;
      }
      ...
      for (var i = 0, l = files.length; i < l; i++) {
        ...
        var mapOrigin = mapObject[filepath] || {};
        var mapTransform = {};
        var keys = Object.keys(mapOrigin);
        for (var k = 0; k < keys.length; k++) {
          mapTransform[keys[k]] = {
            'special': special || self._specialMap[keys[k]],
            'value': mapOrigin[keys[k]]
          };
        }
  ...
  Module.prototype._evaluate = function (content, filename) {
    ...
    // script source url.
    var sourceMap = '//@ sourceURL=' + this._box._sourceUrl(filename, this._isSpecial);
  ...
  function require(name) {
    ...
    var requireSpecial = this._box._specialMap.hasOwnProperty(name);

    // In case `name` is a special and `this` is a normal, resolved will
    //  just return the input. So in order to get the actual value, use
    //  the special map. Note that the reason for calling resolve at all
    //  is to let it throw in case of issues.
    if (requireSpecial) resolved = this._box._specialMap[name];

    // Determine if this request points to a special module
    var fromSpecial = this._isSpecial || requireSpecial;
    var container = fromSpecial ? this._box._special : this._box._normal;
  ...
  function resolve(name) {
    ...
    // Check if resolve name is a known special
    if (this._box._specialMap.hasOwnProperty(name)) {
      item = { 'special': true, 'value': this._box._specialMap[name] };
    }
