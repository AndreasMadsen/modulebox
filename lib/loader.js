...
(function (undefined) {
  ...
  function ModuleBox(settings) {
    ...
    this.require.ensure = function ensure() {
      ensure.apply(self, arguments);
    };
    this.require.resolve = function resolve() {
  ...
  ModuleBox.prototype._request = function (from, request, callback) {
    ...
    xhr.onreadystatechange = function () {
      ...
      if (xhr.status !== 200 && xhr.status !== 304) {
        callback(new Error('Got faulty status code ' + xhr.status), null);
      }
  ...
  Module.prototype._compile = function () {
    ...
    var extname = extname(filename);

    // Get compiler and default to .js
    var compiler = Module._extension[extname];
    if (!compiler) compiler = Module._extension['.js'];
  ...
  function require(name) {
    ...
    return this._box._modules[filepath]._load();
  }
