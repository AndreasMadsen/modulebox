...
ModuleBox.prototype._getMeta = function (filepath, acquired, callback) {
  ...
  // Copy acquired since it will be filled with new filepaths
  acquired = acquired.slice(0);

  var allfound = (function deept(filepaths) {
    for (var i = 0; i < filepaths.length; i++) {
      // filepath must be a string (otherwice resolve error)
      if (typeof filepaths[i] !== 'string') continue;

      // filepath must be new (not already acquired)
      if (acquired.indexOf(filepaths[i]) !== -1) continue;

      // If resource hasn't been cached stop now
      var cache = this._getCache(filepaths[i]);
      ...
      var allfound = deept([cache.resolve]);
  ...
  if (!allfound) return callback(null, null);

  // Create new hash object
  var hash = Buffer.concat(hashes);
  startpoint(hash).pipe(crypto.createHash('sha256')).pipe(endpoint(function (err, hash) {
