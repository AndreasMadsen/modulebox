...
function BundleStream(box, req, res) {
  stream.Transform.call(this);
  var self = this;

  // Split the query
  var query = url.parse(req.url, true).query;
  if (query.from) query.from = JSON.parse(query.from);
  if (query.special) query.special = JSON.parse(query.special);
  if (query.normal) query.normal = JSON.parse(query.normal);
  if (query.request) query.request = JSON.parse(query.request);
  var from = typeof query.from === 'string' ? path.dirname(query.from) : '/';
  var normal = Array.isArray(query.normal) ? query.normal : [];
  var special = Array.isArray(query.special) ? query.special : [];
