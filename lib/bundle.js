...
function BundleStream(box, from, request, acquired) {
  ...
  this.box._localizer(from, request, function (err, resolved) {
    ...
    self._getMeta(function (err, meta) {
      ...
      // Set HTTP headers is possible
      var cacheValid = self._setHeaders();

      // end response if client cache is valid
      if (cacheValid) return self.end();
...
// set HTTP header (called after meta is found)
BundleStream.prototype._setHeaders = function () {
  var valid = false;

  if (this._pipes.res && this._pipes.req) {
    // Set Content-Type header
    this._pipes.setHeader('Content-Type', 'application/xml; charset=utf-8');

    // Set and check cache headers
    valid = notmodified(this._pipes.res, this._pipes.req, this._meta);
