...
function BundleStream(box, from, request, acquired, special) {
  ...
  this.box._resolve(from, request, function (err, resolved) {
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      if (typeof result === 'string') {
        ...
        if (self._acquired.indexOf(result) === -1 &&
            self._query.indexOf(result) === -1) {
          self._query.push(result);
        }
...
BundleStream.prototype._getMeta = function (callback) {
  this.box._getMeta(this._resolve, this._acquired, callback);
};
...
BundleStream.prototype._writeFileStart = function () {
  if (!this._prepend) return;
  this._prepend = false;

  // writes: <file path="{{filepath}}"><![CDATA[
  this.push(DATA_CHUNK.file.startLeft);
  this.push(new Buffer( this._filepath ));
  this.push(DATA_CHUNK.file.startRight);
};

BundleStream.prototype._writeFileEnd = function () {
  if (!this._append) return;
  this._append = false;

  // writes: ]]></file>
  this.push(DATA_CHUNK.file.end);
};
...
BundleStream.prototype._push = function (filepath, dependencies) {
  ...
  this._map[filepath] = dependencies;
...
BundleStream.prototype._next = function () {
  ...
  var file = this._file = fs.createReadStream(realpath);
    ...
    async.parallel({
      ...
      resolved: function (callback) {
        file.pipe(endpoint(function (err, content) {
          ...
          self.box._scanFile(filepath, content, function (err, resolved) {
            if (err) {
              self.emit('error', err);
              return callback(null);
            }
            ...
            for (var i = 0; i < keys.length; i++) {
              ...
              if (typeof result === 'string') {
                storeage[input] = result;
              } else {
                self.emit('warning', result);
                storeage[input] = packError(result);
              }
            }

            // save resolved dependencies in cache
            callback(null, storeage);
          });
        }));
      }
    }, function (err, data) {
