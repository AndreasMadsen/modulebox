...
function BundleStream(box, from, request, acquired) {
  ...
  this.box._resolve(from, request, function (err, resolved) {
    ...
    for (var i = 0; i < resolved.length; i++) {
      ...
      if (result.error) {
        self.emit('warning', result.error);

        // Add result object and remove the filepath and error.stack properties
        self._resolve.push({
          'input': result.input,
          'error': {
        ...
        // to the query list
        if (self._acquired.indexOf(result.filepath) === -1 &&
            self._query.indexOf(result.filepath) === -1) {
          self._query.push(result.filepath);
        }
...
BundleStream.prototype._writeHeader = function (send) {
  ...
  send(new Buffer( JSON.stringify(this._resolve) ));
...
BundleStream.prototype._writeFooter = function (send) {
  ...
  send(new Buffer( JSON.stringify(this._map) ));
